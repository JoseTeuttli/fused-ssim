...
def configure_cuda():
    ...
    compiler_args = {"cxx": ["-O3", "/std:c++17", "/permissive-"], "nvcc": ["-O3"]}
...
def configure_mps():
    ...
    compiler_args = {"cxx": ["-O3", "-std=c++20", "-ObjC++", "-Wno-unused-parameter", "/permissive-"]}
...
def configure_xpu():
    ...
    compiler_args = {"cxx": ["-O3", "-std=c++20", "-fsycl", "/permissive-"]}
    link_args = ["-fsycl"]
